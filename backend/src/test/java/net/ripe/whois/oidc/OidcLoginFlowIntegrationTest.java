...
class OidcLoginFlowIntegrationTest extends AbstractIntegrationTest {
    ...
    @Autowired
    private SessionEmitterService sessionEmitterService;
    @Autowired
    ...
    void resetCaches() {
        ...
        hazelcastInstance.getMap(OidcUtils.HTTP_SESSION_CACHE).clear();
        hazelcastInstance.getMap(OIDC_SESSIONS_MAP).clear();
    ...
    void login_sendsPkce_and_populates_all_Caches() {
        ...
        final Map<Object, Object> sessions = hazelcastInstance.getMap(OidcUtils.HTTP_SESSION_CACHE);
    ...
    void subscribe_while_connected_then_cache_expires_triggers_push_event() throws Exception {
        ...
        await().atMost(5, TimeUnit.SECONDS)
                ...
                .until(() -> sessionEmitterService.hasActiveEmitter(sessionId));
    ...
    void two_users_produce_two_separate_cacheEntries() {
        ...
        Map<Object, Object> sessions = hazelcastInstance.getMap(OidcUtils.HTTP_SESSION_CACHE);
    ...
    void backChannelLogout_removes_session_from_caches_and_expires() throws ExecutionException, InterruptedException,
        ...
        assertThat((Map<Object, Object>) hazelcastInstance.getMap(OidcUtils.HTTP_SESSION_CACHE), aMapWithSize(1));
        ...
        assertThat((Map<Object, Object>) hazelcastInstance.getMap(OidcUtils.HTTP_SESSION_CACHE), anEmptyMap());
