...
export class CreateService {
    ...
    public createRoleMntner(source: string, multipleWhoisObjects: any) {
        return this.http.post(`api/whois-internal/api/mntner-pair/${source.toUpperCase()}/role`, multipleWhoisObjects)
            .pipe(
                map((result: any) => {
    ...
    public createPersonMntner(source: string, multipleWhoisObjects: any) {
        return this.http.post(`api/whois-internal/api/mntner-pair/${source.toUpperCase()}/person`, multipleWhoisObjects)
            .pipe(
                map((result: any) => {
                    console.debug("createRoleMntner success:" + JSON.stringify(result));
                    return result;
