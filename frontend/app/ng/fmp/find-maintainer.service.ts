...
export class FindMaintainerService {
                    ...
                    return this.validate(maintainerKey)
                        .pipe(
                            flatMap((validationResult: {"expired": boolean}) => {
                                return of(this.getFoundMaintainer(result.objects.object[0],
    ...
    private getFoundMaintainer(selectedMaintainer: IWhoisObjectModel, expired: boolean): IFindMaintainer {
        ...
        const maintainerKey = selectedMaintainer.attributes.attribute
            .filter((attr) => attr.name.toLocaleLowerCase() === "mntner")[0]
            .value;
        return {
            maintainerKey,
            selectedMaintainer,
