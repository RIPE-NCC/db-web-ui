...
export class CreateModifyComponent {
    ...
    private fetchDataForModify() {
        ...
        forkJoin({
            ...
            .subscribe(({mntners, objectToModify}: any) => {
                this.restCallInProgress = false;
                console.debug("[createModifyController] object to modify: " + JSON.stringify(objectToModify));

                // store mntners for SSO account
                this.maintainers.sso = mntners;
                console.debug("maintainers.sso:" + JSON.stringify(this.maintainers.sso));

                // store object to modify
                this.attributes = objectToModify.getAttributes();
