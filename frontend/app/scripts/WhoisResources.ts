class WhoisResources {
    public static $inject = ["$log", "WhoisMetaService"];
    ...
    public getFilterableAttrsForObjectTypes(targetObjectTypes: any) {
        const attrsToFilterOn: IAttributeModel[] = [];
        _.each(targetObjectTypes, (objectType) => {
            _.each(this.WhoisMetaService._getMetaAttributesOnObjectType(objectType.toLowerCase(), false), (metaAttr) => {
...
//     var getObjectType = function () {
    public getObjectType = (whoisResponse: IWhoisResponseModel) => () => {
        ...
        if (obj.type) {
            ...
            this.$log.error("No object type found for " + JSON.stringify(this));
        }
        return objectType;
    }
    // private
    ...
    public isValidWhoisResources(whoisResources: IWhoisResponseModel) {
        ...
        if ((_.isUndefined(whoisResources.objects)       || _.isNull(whoisResources.objects))
            ...
            this.$log.error("isValidWhoisResources: Missing objects and errormessages:" + JSON.stringify(whoisResources));
            return false;
        }
        ...
        if (!_.isUndefined(whoisResources) && this.isValidWhoisResources(whoisResources)) {
            ...
            if (!_.isUndefined(objectType) && !_.isUndefined(wrapped.getAttributes())) {
                wrapped.objects.object[0].attributes.attribute =
                    this.wrapAttributes(this.WhoisMetaService.enrichAttributesWithMetaInfo(objectType, wrapped.getAttributes()));
            }
            result = wrapped;
        }
        return result;
...
angular.module("dbWebApp")
    .service("WhoisResources", WhoisResources);
