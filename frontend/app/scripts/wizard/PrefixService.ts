class PrefixService {
    ...
    public isValidIpv4Prefix(str: string): boolean {
        ...
        if (ip4.isValid()) {
            return this._isValidIp4Cidr(ip4);
        }
        return false;
    }
    ...
    public isValidIpv6Prefix(str: string) {
        ...
        if (ip6.isValid()) {
            return this._isValidIp6Cidr(ip6);
        }
        return false;
    }
    ...
    private _isValidIp6Cidr(address: any) {
        // check the subnet mask is in range
        if (!address.parsedSubnet) {
            return false;
        }
...
angular.module("dbWebApp").service("PrefixService", PrefixService);
