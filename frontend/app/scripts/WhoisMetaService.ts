class WhoisMetaService {
    ...
    public _getMetaAttributesOnObjectType(objectTypeName: string, mandatoryOnly: boolean) {
    ...
    public enrichAttributesWithMetaInfo(objectTypeName: string, attrs: IAttributeModel[]) {
        ...
        const attrsMeta = this._getMetaAttributesOnObjectType(objectTypeName, false);
        ...
        _.each(attrs, (attr: IAttributeModel) => {
            ...
            if (!_.isUndefined(attrMeta)) {
                if (!_.isUndefined(attr.$$meta)) {
                    ...
                    enrichedAttrs.push(this._wrapMetaInAttribute(this, objectTypeName, attr.name, value, attr.comment, attr.link, attr["referenced-type"], attrMeta, undefined));
                }
            }
        });
    ...
    public getAllAttributesOnObjectType(objectTypeName: string): IAttributeModel[] {
        ...
        let idx = 0;
        return _.map(this._getMetaAttributesOnObjectType(objectTypeName, false), (meta: any) => {
            const wrapped = this._wrapMetaInAttribute(this, objectTypeName, meta.name, undefined, undefined, undefined, undefined, meta, idx);
            idx++;
            return wrapped;
        });
    }
    ...
    public getMandatoryAttributesOnObjectType(objectTypeName: string) {
        ...
        // enrich with order info
        let idx = 0;
        return _.map(this._getMetaAttributesOnObjectType(objectTypeName, true), (meta: any) => {
            const wrapped = this._wrapMetaInAttribute(this, objectTypeName, meta.name, "", undefined, undefined, undefined, meta, idx);
            idx++;
            return wrapped;
        });
    }
    ...
    private _getDocumentationForAttribute(objectType: string, attrName: string, docKind: string) {
        ...
        if (doc === null) {
            this.$log.info("objectType, attrName, docKind", objectType, attrName, docKind);
        } else if (!_.isUndefined(doc)) {
            return this._lookupDefaultText(doc, objectType, attrName, docKind);
        } else {
            this.$log.warn("No documentation for objectType:", objectType, "attrName:", attrName);
        }
        return doc;
    }

    private _lookupDefaultText(doc: any, objectType: string, attrName: string, docKind: string) {
    ...
    private _wrapMetaInAttribute(self: any, objectTypeName: string, attrName: string, attrValue: string, attrComment: string,
                                 attrLink: IWhoisLinkModel, reffedAttrType: string, metaAttribute: any, idx?: number) {
        return {
            "$$meta": {
                $$idx: idx,
    ...
    private _shared = {
        ...
        irt: {
            syntax: "An irt name is made up of letters, digits, the underscore \"_\" and hyphen \"-\". It must start with" +
        ...
        nicHandle: {
            syntax: "From 2 to 4 characters, followed by up to 6 digits and a source specification. " +
        ...
        organisationName: {
            syntax: "A word may have up to 64 characters and is not case sensitive. " +
    ...
    private _attrDocumentation = {
        ...
        "mp-filter": {
            ...
            // short: undefined,
            syntax: "Logical expression which when applied to a set of multiprotocol routes returns a subset of these routes. " +
...
angular.module("dbWebApp").service("WhoisMetaService", WhoisMetaService);