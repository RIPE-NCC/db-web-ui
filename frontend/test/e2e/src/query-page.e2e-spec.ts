...
describe('The query page', () => {
    ...
    it('should not uncheck domain flag when hierarchical flag is unselected', async () => {
        ...
        expect(page.hierarchyFlagSlider.getAttribute('aria-valuenow')).toEqual('0');
        // move slider to x
        await browser.actions().dragAndDrop(page.hierarchyFlagSlider, { x: 500, y: 0 }).perform();
        ...
        // move slider to x
        await browser.actions().dragAndDrop(page.hierarchyFlagSlider, { x: 3, y: 0 }).perform();
    ...
    it('should contain ripe-nonauth for source in link on attribute value', async () => {
        ...
        await page.scrollIntoCenteredView(page.btnSubmitQuery);
        page.btnSubmitQuery.click();
        expect(page.getAttributeHrefFromWhoisObjectOnQueryPage(2, 0).getAttribute('href')).toContain(
            '?source=ripe-nonauth&key=211.43.192.0%2F19AS9777&type=route',
        );
    });
    ...
    it('should be able to show out of region route from ripe db without related objects', async () => {
        ...
        page.btnShare.click();
        // XML
        expect(page.linksToXmlJSON.get(0).getAttribute('href')).toContain(
            '.xml?query-string=AS9777&type-filter=aut-num&flags=no-referenced&flags=no-irt&flags=no-filtering&source=RIPE',
        );
        // JSON
        expect(page.linksToXmlJSON.get(1).getAttribute('href')).toContain(
            '.json?query-string=AS9777&type-filter=aut-num&flags=no-referenced&flags=no-irt&flags=no-filtering&source=RIPE',
        );
    });
    ...
    it('should show query-flags-container with adequate flags', async () => {
        await page.scrollIntoCenteredView(page.inpQueryString);
        page.inpQueryString.clear().sendKeys('-i abuse-c -T organisation -Br --sources RIPE ANNA');
        const allExpectFlags = ['-i', '--inverse', '-T', '--select-types', '-B', '--no-filtering', '-r', '--no-referenced', '-s', '--sources'];
