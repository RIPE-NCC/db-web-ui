...
describe("ConfirmMaintainerComponent", () => {
    ...
    it("should inform user mntner already associated with current user", () => {
        ...
        expect(component.alertsService.alerts.errors.length).toBe(0);
        expect(component.alertsService.alerts.warnings[0].plainText)
    ...
    it("should return message if associate is cancelled", () => {
        ...
        expect(component.alertsService.alerts.warnings[0].plainText).toBe(`<p>No changes were made to the <span class="mntner">MNTNER</span> object .</p><p>If you wish to add a different RIPE NCC Access account to your <strong>MNTNER</strong> object:<ol><li>Sign out of RIPE NCC Access.</li><li>Sign back in to RIPE NCC Access with the account you wish to use.</li><li>Click on the link in the instruction email again.</li></ol>`);
    });
