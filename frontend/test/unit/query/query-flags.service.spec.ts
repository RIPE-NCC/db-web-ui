...
describe("QueryFlagsService", () => {
    ...
    it("should detect multple flags", () => {
        queryFlagsService.getFlags(["-i", "-q", "--show-tag-info"])
            .subscribe(flags => {
    ...
    it("should return just valid query flags", () => {
        queryFlagsService.getFlags(["-zg", "-i"])
            .subscribe(flags => {
                expect(flags).toEqual([{
                    "longFlag": "--inverse",
                    "shortFlag": "-i",
    ...
    it("should return just valid query flag from grouped flags", () => {
        queryFlagsService.getFlags(["-zb"])
            .subscribe(flags => {
                expect(flags).toEqual([{
                    "longFlag": '--abuse-contact',
                    "shortFlag": '-b',
                    "description": 'Requests the "abuse-mailbox:" address related to the specified inetnum or inet6num object. Only specified object key and "abuse-mailbox:" attributes are shown.'
                }]);
            });
        const request = httpMock.expectOne({method: "GET", url: "api/metadata/help"});
        expect(request.request.method).toBe("GET");
    ...
    it("should return query flags for grouped flags", () => {
        queryFlagsService.getFlags(["-xdBr", "--sources"])
            .subscribe(flags => {
                expect(flags).toEqual([{
                    "longFlag": "--exact",
                    "shortFlag": "-x",
...
const mockAllFLAGS: IQueryFlag[] = [
    ...
    },
    {
        "longFlag": "--irt",
        "shortFlag": "-c",
        "description": "Requests first level less specific inetnum or inet6num objects with the \"mnt-irt:\" attribute. Related irt objects are not returned by default."
    },
    {
        "longFlag": "--abuse-contact",
        "shortFlag": "-b",
    ...
    },
    {
        ...
        "shortFlag": "-K",
        "description": "Requests that only the primary keys of an object to be returned. The exceptions are set objects, where the (mp-)members attributes will also be returned. This flag does not apply to person and role objects."
    },
    {
        "longFlag": "--persistent-connection",
        "shortFlag": "-k",
        "description": "Requests a persistent connection. After returning the result the connection will not be closed by the server and a client may issue multiple queries on the same connection.\nNote, that server implements 'stop-and-wait' protocol, when no next query can be sent before receiving a reply for the previous one.\nExcept the first -k query, -k without an argument closes the persistent connection."
    },
    {
    ...
    },
    {
        ...
        "shortFlag": "",
        "description": "Returns the current set of sources along with the information required for mirroring. See [REF], section 2.9 \"Other server features\" for more information."
    },
    {
        "longFlag": "--version",
        "shortFlag": "",
